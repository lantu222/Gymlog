...
module.exports = [
  {
    ...
    run() {
      ...
      assert.match(onboardingSource, /bodyweightTargetStepButton:\s*\{[\s\S]*width: 42[\s\S]*height: 42/);
      ...
      assert.match(onboardingSource, /bodyweightTargetStepGlow:\s*\{[\s\S]*backgroundColor: '#8B5CF6'/);
      ...
      assert.match(onboardingSource, /const targetWeightBlock = \([\s\S]*<BodyweightStepper[\s\S]*label="Goal weight \(optional\)"[\s\S]*onChange=\{\(nextValue\) =>/);
      assert.doesNotMatch(onboardingSource, /const targetWeightBlock = \([\s\S]*<BodyweightTargetSlider/);
      assert.match(onboardingSource, /bodyweightGoalSummaryPill/);
