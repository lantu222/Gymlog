...
This document defines the hard boundaries between Vinha's subsystems. These are not style guidelines — they are constraints enforced at the architectural level. A boundary violation is a bug, not a design choice.
...
- Calling the Vinha AI layer for any part of programme selection or construction
...
- Calling the Vinha AI layer to select or construct a programme
...
**Forbidden:**
- Showing any Vinha AI output during an active session — absolutely none, including exercise substitution suggestions surfaced without user request
...
**Boundary rule — ADR-001:** No AI output of any kind may be surfaced while a workout session is active. This is architecturally enforced: `computePostSessionInsight()` and the Vinha AI client must not be called while `WorkoutProvider` has an active session.
...
- Producing output while there is no post-session context (the Vinha AI is not a chatbot on-demand for arbitrary queries in MVP)

**Boundary rule:** The Vinha AI explains and advises. It does not generate programmes, select templates, or modify the user's training plan directly.
...
User completes onboarding inputs
  ...
  → NEVER: call Vinha AI to select or explain the programme during onboarding
