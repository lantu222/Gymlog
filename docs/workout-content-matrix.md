...
The ready plan must be constructible from the current five onboarding steps without AI generation. The engine can use Vinha AI later to explain or discuss the plan, but the template, session content, and starter block come from deterministic product data.
...
If any of these fields cannot be generated from catalog data, the issue is a content gap, not something Vinha AI should invent.
