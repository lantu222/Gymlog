...
During active workout sessions, the user's attention is on training. Any Vinha AI output — whether an insight, a suggestion, a substitution prompt, or an encouragement message — competes with that attention and constitutes an interruption.
...
**All Vinha AI output is architecturally prohibited during active workout sessions. No exceptions.**
...
5. **Substitution suggestions are a user-initiated action.** If a user requests a substitution (by tapping a substitution button), that is user-initiated and not in-session Vinha AI. This decision prohibits *unsolicited* AI output only. User-initiated exercise substitution lookup is allowed.

---

## Implementation Constraint

`computePostSessionInsight()` and the Vinha AI client (`aiCoachClient.ts`) must only be called after the session save resolves. The `WorkoutProvider` active session state is the gate.
