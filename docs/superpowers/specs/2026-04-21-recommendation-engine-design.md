...
# Vinha Recommendation Engine Design (ARCHIVED)

## Context

Vinha already has four overlapping product surfaces that influence training choice:
...
- let Vinha AI explain and later adapt within explicit guardrails as a separate premium layer

This design assumes Vinha keeps supporting all current entry paths:

- start an empty workout
- build your own template
- choose ready templates / ready plans
- accept a recommendation from onboarding
- later use Vinha AI for explanation, comparison, and adaptation
...
3. How can Vinha explain that recommendation in a way that feels tailored instead of generic?
...
- Vinha keeps its current distinction between ready programme content, custom templates, and Vinha AI surfaces.
...
- Vinha AI can explain and safely adjust recommendations, but does not become the system of record for training logic.
- Recommendation output should be reusable in first run, Vinha AI, and later "change my plan" flows.
...
- Vinha AI experience in `src/screens/AICoachScreen.tsx`, `src/lib/aiCoachActions.ts`, and `src/lib/aiCoachPlan.ts`
...
- ask Vinha AI for explanation or a guarded variation if premium is active
...
- confidence layer
- optional Vinha AI explanation and limited premium adaptation
...
## Vinha AI Interaction Rules

Vinha AI should be useful because it makes the engine understandable and adaptive, not because it ignores the engine.
...
Vinha AI can recommend re-ranking or switching when:
...
- Vinha AI conversations tagged with:
  - too long
  - too hard
  - boring
  - hurts
...
If Vinha AI can improvise too freely, users will get inconsistent recommendations and developers will lose debuggability.

Decision:

- keep Vinha AI as interpreter and guarded adapter, not recommendation engine of record
...
This will make Vinha feel personalized without becoming unpredictable. The user should come away feeling that Vinha understood:
