# Vinha Project Context

Tama tiedosto on projektin "AI-muisti": lue tama ensin, jos et tunne aiempaa keskusteluhistoriaa.

## Project Purpose

Vinha is a mobile-first workout planning and logging app. The product goal is to make training feel decisive and practical:
...
- provide Vinha AI guidance without making the core app depend on a live AI backend
...
- Optional serverless Vinha AI endpoint in `api/ai-coach.ts`
...
- The app should work locally without a backend. Vinha AI has preview mode when `EXPO_PUBLIC_AI_COACH_API_URL` is not configured.
- Live Vinha AI should call a project-owned endpoint, which then calls OpenAI. Do not call OpenAI directly from the mobile app.
- Prompt text and training context should not be intentionally logged by the Vinha AI endpoint.
...
- Vinha AI preview/live guidance
...
- Make Vinha AI operational with scoped actions that can move users into real app flows.
...
|-- api\
|   `-- ai-coach.ts                 Optional serverless Vinha AI endpoint
