...
| Question | UI section |
| --- | --- |
| What plan did Vinha pick? | Hero title and subtitle |
...
Use existing `VinhaIcon` names where possible. If an exact icon is missing, map to the closest existing icon first before adding new icon paths.
...
- Reuse `VinhaIcon` wherever possible.
