...
5. [AI Coaching Boundaries](#5-ai-coaching-boundaries)
...
- Context assembly for AI coaching calls
...
When the AI coach is invoked, the context assembler selects the most relevant subset of memory for the current situation. It does not dump everything — it curates.
...
| AI coaching (preview mode) | Fully functional (local mock) |
| AI coaching (live mode) | Degrades to silence — no error shown |
...
**Debugging is possible.** Any unexpected AI coaching output can be traced back to the exact log data that triggered it. This is only possible if the log is never mutated.
...
### Priority 4: AI coaching isolation

The AI coach must degrade gracefully when the network is unavailable. Preview mode must be indistinguishable in behavior from live mode (same types, same interfaces).

**Architecture requirements:**
- AI coach interface is identical in preview and live modes
...
**Current state:** AI coaching messages are generated by an LLM with a structured prompt.
...
**Mitigation path:** Supabase is not introduced until: (1) the core workout logging experience is stable, (2) at least one AI coaching feature is complete, and (3) the user base justifies cross-device sync.
...
**Mitigation path:** Instrument insight delivery (type, confidence, phase) in analytics from day one of AI coach activation. Adjust thresholds based on observed data — do not guess.
