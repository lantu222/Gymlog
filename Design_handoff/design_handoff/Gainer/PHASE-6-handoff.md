...
**Theme pattern (confirmed against the codebase):** Phases 1–5 did NOT flip the global `theme.ts` or build a shared light module — each light screen defines its own local palette block (e.g. `HOME_BACKGROUND = '#F7F3FF'` at the top of `HomeScreen.tsx`). **Follow that pattern: per-screen local light tokens.** Do NOT flip the global dark `theme.ts`. (Optional, only if trivially low-risk: a tiny shared light-tokens constant imported by just the five new screens to avoid drift — but skip it if it means touching shared dark code. `MainScreenPrimitives` SurfaceCard/BadgePill are still dark, so Premium/Progress get no free light styling from them.)
...
7. **PREFERENCES:** segmented controls **Units (kg/lb)**, **Language (FIN/ENG)**, **Theme (Light/Dark)**. **OMIT the Notifications toggle for v1** — push notifications don't exist (`mvp-launch-scope.md` §4.1/§7.4: no toggle for a feature that isn't built). Wire the rest to existing `AppPreferences` / `onPreferencesChange` / unit handler.
...
## SCREEN D — Premium  *(restyle only — NO paywall in v1)*
**Target:** `src/screens/PremiumScreen.tsx` (452 lines). **Restyle to the light design only.**
**Design:** `GAINER Premium.html` / `premium-app.jsx` (visual reference for layout/hero/lanes/comparison).

> ⚠️ **SCOPE OVERRIDE (decided in review):** `mvp-launch-scope.md` §4.6/§8.5 exclude the premium paywall from v1 — **no subscription gate, no plan selector, no pricing, no "Start 7-day free trial" purchase CTA, no RevenueCat.** The scope doc is authoritative and overrides this handoff's earlier RevenueCat instruction. Today's `PremiumScreen.tsx` is already paywall-free (informational + `previewUnlocked` toggle) — keep it that way.

What to build: restyle the existing informational screen to the light tokens. You MAY keep, as informational content: the hero ("Keep progressing — without the guesswork" + the squat-progression chart via `react-native-svg`), the **WHAT PREMIUM ADDS** lanes with `LIVE`/`SOON` tags (`premiumLanes` already exists), and the **FREE VS PREMIUM** comparison (`comparisonRows` already exists). **Drop** the CHOOSE-YOUR-PLAN cards and all prices. Replace the trial CTA with the existing **"Try the preview"** action (the `previewUnlocked` toggle). Keep `react-native-svg` for the hero chart only.

Ignore `GAINER Premium - build notes.md` (RevenueCat) for v1 — it's deferred until premium actually ships. Reuse existing `preferences.adaptiveCoachPremiumUnlocked` / `AccessTier` as-is; don't add purchase logic.
...
- **Measures:** selected-measure detail (big value + delta pill + chart); ALL MEASURES list (sparkline + delta pill, with the "lower-is-better" inversion for waist/bodyfat). **DROP the PROGRESS PHOTOS section for v1** — don't show UI for a feature that isn't built (Rule 7 / scope).
...
3. **Profile** — large but low-risk (presentational + existing prefs). Omit Notifications toggle.
4. **Premium** — **restyle only, no paywall/RevenueCat** (scope override above).
5. **Progress** — biggest/riskiest, last. Drop Progress Photos.
...
- **Premium paywall is deferred** (out of v1 scope). When premium ships, wire RevenueCat per `GAINER Premium - build notes.md` and add back the plan selector / pricing / trial CTA.
- **Notifications & Progress Photos** are omitted in v1 (features don't exist yet); add their UI when the features land.
